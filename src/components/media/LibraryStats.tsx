...
export default function LibraryStats({ totalTB, movieCount, seriesCount, albumCount, bookCount, timeLabel, libraries }: LibraryStatsProps) {
    ...
    const getIconPrefix = (collectionType?: string | null, name?: string) => {
        ...
        return <Library className="w-5 h-5 text-muted-foreground shrink-0" />;
    };
    ...
    const contentWithSeparators = _contentItems.flatMap((item, i) => i === 0 ? [item] : [<span key={`sep-${i}`} className="text-muted-foreground font-normal">, </span>, item]);
    ...
    return (
        <div className="space-y-8 mb-10 mt-6">
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 pt-8 pb-2 border-b border-border/70">
                    ...
                    return (
                                    ...
                                    <div className="flex items-center justify-between gap-2 w-full">
                                        <div className="flex items-center gap-2 w-full">
                                            <div className="flex items-center gap-2 min-w-0">
                                                ...
                                                <div className="flex items-center gap-2 min-w-0">
                                                    <CardTitle className="text-xl font-bold pr-2 text-foreground line-clamp-2 whitespace-normal break-words">{displayName}</CardTitle>
                                                    {lib.name && lib.name !== displayName ? (
                                                        <span className="app-chip text-xs ml-1 px-2 py-0.5 rounded-full">{lib.name}</span>
                                                    ) : null}
                                                </div>
                                            </div>
                                            <div className="flex items-center gap-2">
                                                <span className="app-chip shrink-0 text-[10px] font-mono font-medium tracking-tighter px-2 py-0.5 rounded-full">
                                                    {lib.size}
                                                </span>
                                            </div>
                                        </div>
                        ...
                        <CardContent className="relative z-10 p-5 pt-3 space-y-4 flex-1 flex flex-col">
                            <div className="flex flex-wrap items-center justify-between gap-2 border-b border-border pb-3">
                                <div className="text-[11px] font-medium text-muted-foreground flex items-center gap-1.5 leading-snug">
                            ...
                            {(lib.ignoredEpisodes || lib.ignoredTracks) ? (
                                <div className="flex items-center gap-4 text-xs text-muted-foreground mt-2">
                                    {lib.ignoredEpisodes ? (
                                        <div className="flex items-center gap-1"><FileVideo className="w-3 h-3 text-muted-foreground" /> {lib.ignoredEpisodes} {tc('episodes').toLowerCase()}</div>
                                    ) : null}
                                    {lib.ignoredTracks ? (
                                        <div className="flex items-center gap-1"><Music className="w-3 h-3 text-muted-foreground" /> {lib.ignoredTracks} {tc('tracks').toLowerCase()}</div>
                                    ) : null}
                                </div>
                            ) : null}
                                ...
                                <div className="space-y-3 flex-1 flex flex-col">
                                    ...
                                    {lib.topItem ? (
                                        <Link 
                                            ...
                                            className="app-surface-nested flex-1 flex items-center gap-3 p-3 rounded-xl border hover:border-primary/30 hover:shadow-md transition-all group/item"
                                        >
                                            ...
                                            <div className="flex-1 min-w-0">
                                                ...
                                                <div className="text-sm font-semibold text-foreground truncate group-hover/item:text-primary transition-colors">{lib.topItem.title}</div>
                                                <div className="text-xs text-muted-foreground mt-0.5">{lib.topItem.plays} {tc('views')}</div>
                                            </div>
                                        </Link>
                                    ) : (
                                        <div className="app-surface-nested flex-1 flex items-center gap-3 p-3 rounded-xl border border-dashed border-border/80">
